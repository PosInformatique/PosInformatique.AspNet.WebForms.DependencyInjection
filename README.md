...
constructor which matches the parameters of the control, or service to instantiate.
...
When the application is loaded the ASP .NET Compiler generate for example the following code which will not add the
[ActivatorUtilitiesConstructorAttribute](https://docs.microsoft.com/en-us/dotnet/api/microsoft.extensions.dependencyinjection.activatorutilitiesconstructorattribute)
...
In the version 1.2.0, the **PosInformatique.AspNet.WebForms.DependencyInjection** add the full support of the
[ActivatorUtilitiesConstructorAttribute](https://docs.microsoft.com/en-us/dotnet/api/microsoft.extensions.dependencyinjection.activatorutilitiesconstructorattribute)
by *virtually* add the attribute in the class generated by the ASP .NET Compiler.

## Issue in the ASPX/ASCX designer

When changing the constructor of the user controls and control to inject dependency using the
[HttpRuntime.WebObjectActivator](https://docs.microsoft.com/fr-fr/dotnet/api/system.web.httpruntime.webobjectactivator),
the following message can be appear when added these controls to a page or other user controls:
...
In this case, lot of *fake* errors/warnings can be raise by Visual Studio which compile and execute the compilation
correctly.
...
This is a known
[issue](https://developercommunity.visualstudio.com/content/problem/668468/user-control-fake-error-with-dependency-injection.html)
of Visual Studio and Microsoft seam to take *Under Consideration*...

There is workaround, which is provided with the **PosInformatique.AspNet.WebForms.DependencyInjection** package by adding
...
You have also to add the [ActivatorUtilitiesConstructorAttribute](https://docs.microsoft.com/en-us/dotnet/api/microsoft.extensions.dependencyinjection.activatorutilitiesconstructorattribute)
